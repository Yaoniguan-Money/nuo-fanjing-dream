...
const cardsDirectory = path.join(projectDirectory, "cards");
const assetsDirectory = path.join(projectDirectory, "assets");
const manifestPath = path.join(assetsDirectory, "assets.manifest.json");
const schemaPath = path.join(projectDirectory, "schemas", "dream-card.schema.json");
const templatePath = path.join(projectDirectory, "create-guide", "dream-card.template.json");
...
async function readJson(filePath) {
  try {
    ...
    errors.push(`${path.relative(projectDirectory, filePath)}: ${error.message}`);
    return null;
  }
}
...
const cardNames = (await readdir(cardsDirectory)).filter((name) => name.endsWith(".json")).sort();
...
if (schema) {
  ...
  for (const filePath of contentPaths) {
    ...
    if (referencedSchema !== schemaPath) {
      errors.push(`${relativeFile(filePath)}: $schema must resolve to schemas/dream-card.schema.json`);
    }
...
if (manifest?.schemaVersion !== "0.1.0") {
  errors.push("assets/assets.manifest.json: schemaVersion must be 0.1.0");
}
if (!manifest || !Array.isArray(manifest.assets)) {
  errors.push("assets/assets.manifest.json: assets must be an array");
} else {
  for (const [index, asset] of manifest.assets.entries()) {
    const label = `assets/assets.manifest.json/assets/${index}`;
    ...
    if (!isInside(assetsDirectory, assetPath) || assetPath.startsWith(`${path.join(assetsDirectory, "source")}${path.sep}`)) {
      errors.push(`${label}: file must stay inside the runtime assets directory and outside assets/source`);
      continue;
    }
    try {
      const assetStat = await stat(assetPath);
      if (!assetStat.isFile()) errors.push(`${label}: ${asset.file} is not a file`);
      if (Number.isInteger(asset.bytes) && asset.bytes !== assetStat.size) {

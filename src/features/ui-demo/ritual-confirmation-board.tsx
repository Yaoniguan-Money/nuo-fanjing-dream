...
function orbitStyle(index: number): CSSProperties {
  ...
  return {
    ...
    "--mask-y": `${48 + y * 31}%`,
    "--mask-scale": String(0.78 + (y + 1) * 0.12),
...
function MaskOrbit({ scene }: { scene: (typeof sceneLabels)[number] }) {
  ...
  return <div className={`ritual-board__orbit${selected ? " is-selected" : ""}`}>
    ...
    >
      <Image src={mask.asset} alt="" fill sizes="18vw" priority={index < 4} />
    </div>)}
  </div>;
}
...
function Scene({ index, label }: { index: number; label: (typeof sceneLabels)[number] }) {
  ...
  return <section className="ritual-board__scene" role="region" aria-label={label}>
    ...
    {label === "匹配命中" ? <div className="ritual-board__copy"><small>此面与你所问相照</small><strong>开路将军</strong><span>职司 · 开障引路</span><b>入 戏</b></div> : null}
    {label === "故事收束" ? <div className="ritual-board__copy ritual-board__copy--collect"><small>幻梦已尽 · 得面已成</small><strong>开路将军</strong><span>职司 · 开障引路</span><b>收 录 此 面</b><i aria-hidden="true" /></div> : null}

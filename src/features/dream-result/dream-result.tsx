...
import { clearDreamSession, readDreamSession, saveInterpretation } from "@/domain/dream-session/storage";
...
type ResultState = { status: "loading" } | { status: "missing" } | { status: "error" } | { status: "ready"; wish: string; interpretation: Interpretation; getFaceSession: GetFaceRitualSession | null };
...
export function DreamResult() {
  ...
  useEffect(() => {
    ...
    const getFaceSession = readGetFaceRitualSession();
    if (session.interpretation) {
      const timer = window.setTimeout(() => { if (active) setState({ status: "ready", wish: session.wish, interpretation: session.interpretation!, getFaceSession: getFaceSession?.phase === "complete" ? getFaceSession : null }); }, 0);
      return () => { active = false; window.clearTimeout(timer); };
    }
    fetch("/api/dream/interpret", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ cardId: session.match.cardId, wish: session.wish }) })
      ...
      .then((response) => { saveInterpretation(response.interpretation); if (active) setState({ status: "ready", wish: session.wish, interpretation: response.interpretation, getFaceSession: getFaceSession?.phase === "complete" ? getFaceSession : null }); })
  ...
  if (state.getFaceSession) return <GetFaceResult session={state.getFaceSession} interpretation={state.interpretation} onRestart={() => { clearDreamSession(); clearGetFaceRitualSession(); router.push("/"); }} />;

...
export function WishInput({ onReturn }: { onReturn: () => void }) {
  ...
  return <main className="wish-page"><form className="wish-form" onSubmit={submit}><span>门 后 · 问 愿</span><h1>你想为哪件事寻一条路？</h1><p>愿望只用于匹配一张现有幻梦卡。选中后，七幕剧情固定，任何 AI 都不能改写。</p><textarea {...register("wish")} autoFocus rows={5} placeholder="例如：我被一个迟迟无法开始的选择困住了……" /><div className="wish-error" role="alert">{errors.wish?.message ?? errors.root?.message}</div><button className="ui-primary-cta" type="submit" disabled={isSubmitting}>{isSubmitting ? "正在问路" : "请 幻 梦"}</button><button className="wish-return ui-return-control" type="button" onClick={onReturn}>← 返回山门</button></form></main>;
}
